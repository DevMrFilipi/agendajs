export default class SvgRender {
    constructor(fill, viewBox, style, pathD) {
      Object.defineProperties(this, {
        fill: {
            enumerable: false,
            configurable: false,
            get: function () {
              return String(fill);
            },
            set: function (inputFill) {
              this.isValidInput(inputFill);
              fill = inputFill;
            },
          },
        ariaHidden: {
          enumerable: false,
          configurable: false,
          get: function () {
            return String(ariaHidden);
          },
          set: function (inputAriaHiden) {
            this.isValidInput(inputAriaHiden);
            ariaHidden = inputAriaHiden;
          },
        },
        viewBox: {
          enumerable: false,
          configurable: false,
          get: function () {
            return String(viewBox);
          },
          set: function (inputViewBox) {
            this.isValidInput(inputViewBox);
            viewBox = inputViewBox;
          },
        },
        classSvg: {
          enumerable: false,
          configurable: false,
          get: function () {
            return String(style);
          },
          set: function (tyle) {
            this.isValidInput(tyle);
            style = tyle;
          },
        },
        pathD: {
          enumerable: false,
          configurable: false,
          get: function () {
            return String(pathD);
          },
          set: function (inputPathD) {
            this.isValidInput(inputPathD);
            pathD = inputPathD;
          },
        },
      });
    }
    isValidInput(input) {
      if (typeof input === undefined || input === "" || input === 0) return;
    }
    createAstSvg() {
      let questionSvg = this.createSvg(
        "1em",
        "0 0 384 512",
        "margin:0;padding:3px;fill:#f8a516;font-size:10px;",
        "M192 32c17.7 0 32 14.3 32 32V199.5l111.5-66.9c15.2-9.1 34.8-4.2 43.9 11s4.2 34.8-11 43.9L254.2 256l114.3 68.6c15.2 9.1 20.1 28.7 11 43.9s-28.7 20.1-43.9 11L224 312.5V448c0 17.7-14.3 32-32 32s-32-14.3-32-32V312.5L48.5 379.4c-15.2 9.1-34.8 4.2-43.9-11s-4.2-34.8 11-43.9L129.8 256 15.5 187.4c-15.2-9.1-20.1-28.7-11-43.9s28.7-20.1 43.9-11L160 199.5V64c0-17.7 14.3-32 32-32z"
      );
      return questionSvg;
    }
    createInfoSvg() {
        let infoSvg = this.createSvg(
            "currentColor",
            "true",
            "0 0 20 20",
            "flex-shrink-0 inline w-4 h-4 me-3",
            "M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z"
        );
        return infoSvg;
    }
    createCheckSvg() {
      let checkSvg = this.createSvg(
        "1em",
        "0 0 448 512",
        "margin:0;padding:5px;fill:green;",
        "M438.6 105.4c12.5 12.5 12.5 32.8 0 45.3l-256 256c-12.5 12.5-32.8 12.5-45.3 0l-128-128c-12.5-12.5-12.5-32.8 0-45.3s32.8-12.5 45.3 0L160 338.7 393.4 105.4c12.5-12.5 32.8-12.5 45.3 0z"
      );
      return checkSvg;
    }
    createWrongSvg() {
      let wrongSvg = this.createSvg(
        "1em",
        "0 0 384 512",
        "margin:0;paddig-inline-start:3px;padding-top:4px;fill:red;",
        "M342.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L192 210.7 86.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L146.7 256 41.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L192 301.3 297.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L237.3 256 342.6 150.6z"
      );
      return wrongSvg;
    }
    createEyeSvg() {
      let eyeSvg = this.createSvg(
        "1em",
        "0 0 576 512",
        "font-size:15px;fill:#000000;margin:23px 0 0 189px;cursor:pointer",
        "M288 32c-80.8 0-145.5 36.8-192.6 80.6C48.6 156 17.3 208 2.5 243.7c-3.3 7.9-3.3 16.7 0 24.6C17.3 304 48.6 356 95.4 399.4C142.5 443.2 207.2 480 288 480s145.5-36.8 192.6-80.6c46.8-43.5 78.1-95.4 93-131.1c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C433.5 68.8 368.8 32 288 32zM144 256a144 144 0 1 1 288 0 144 144 0 1 1 -288 0zm144-64c0 35.3-28.7 64-64 64c-7.1 0-13.9-1.2-20.3-3.3c-5.5-1.8-11.9 1.6-11.7 7.4c.3 6.9 1.3 13.8 3.2 20.7c13.7 51.2 66.4 81.6 117.6 67.9s81.6-66.4 67.9-117.6c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3z"
      );
      return eyeSvg;
    }
    createEyeSlashSvg() {
      let eyeSlashSvg = this.createSvg(
        "1em",
        "0 0 640 512",
        "font-size:15px;fill:#000000;margin:23px 0 0 189px;cursor:pointer",
        "M38.8 5.1C28.4-3.1 13.3-1.2 5.1 9.2S-1.2 34.7 9.2 42.9l592 464c10.4 8.2 25.5 6.3 33.7-4.1s6.3-25.5-4.1-33.7L525.6 386.7c39.6-40.6 66.4-86.1 79.9-118.4c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C465.5 68.8 400.8 32 320 32c-68.2 0-125 26.3-169.3 60.8L38.8 5.1zM223.1 149.5C248.6 126.2 282.7 112 320 112c79.5 0 144 64.5 144 144c0 24.9-6.3 48.3-17.4 68.7L408 294.5c8.4-19.3 10.6-41.4 4.8-63.3c-11.1-41.5-47.8-69.4-88.6-71.1c-5.8-.2-9.2 6.1-7.4 11.7c2.1 6.4 3.3 13.2 3.3 20.3c0 10.2-2.4 19.8-6.6 28.3l-90.3-70.8zM373 389.9c-16.4 6.5-34.3 10.1-53 10.1c-79.5 0-144-64.5-144-144c0-6.9 .5-13.6 1.4-20.2L83.1 161.5C60.3 191.2 44 220.8 34.5 243.7c-3.3 7.9-3.3 16.7 0 24.6c14.9 35.7 46.2 87.7 93 131.1C174.5 443.2 239.2 480 320 480c47.8 0 89.9-12.9 126.2-32.5L373 389.9z"
      );
      return eyeSlashSvg;
    }
    createSvg(fill, ariaHidden, viewBox, classSvg, pathD) {
      let svg = this.createElementNS("svg");
      let path = this.createElementNS("path");
      svg.setAttribute("fill", fill);
      svg.setAttribute("aria-hidden", ariaHidden);
      svg.setAttribute("viewBox", viewBox);
      svg.setAttribute("class", classSvg);
      path.setAttribute("d", pathD);
      svg.appendChild(path);
      return svg;
    }
    createElementNS(element) {
      let el = document.createElementNS("http://www.w3.org/2000/svg", element);
      return el;
    }
  }